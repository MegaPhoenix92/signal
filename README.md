...
**Tenant advantage:** public research (filled by an **automatic** `public_sales_signal` worker) and marketing-scrub automations may *update the tenant DB*. Tenant mail never feeds public research. The public worker is required for P5 and is isolated from tenant mailbox detection.
