import { useEffect, useMemo, useState, type CSSProperties, type DependencyList, type FormEvent, type ReactNode } from 'react';
import {
  Activity,
  AlertTriangle,
  ArrowRight,
...
type AdminTab = 'overview' | 'users' | 'email' | 'governance' | 'integrations' | 'payments' | 'ops' | 'cli';
...
type LiveState = {
  ...
  claimInvite: (args: Record<string, unknown>) => Promise<void>;
  lastMutation: string | null;
  mutate: (action: SignalMutationAction, args: Record<string, unknown>) => Promise<void>;
  providerReadiness: ProviderReadiness;
  providerSandbox: ProviderSandboxReport | null;
  registerWorkspace: (args: Record<string, unknown>) => Promise<void>;
...
function RegistrationOnboarding({ liveState }: { liveState: LiveState }) {
  ...
  }, [pendingInvites.length, selectedInvite?.id]);
  ...
  async function acceptSelectedInvite() {
    ...
    await mutate('users.invite-accept', {
      ...
      <main className="product-main">
        ...
        <section className="registration-grid">
          <article className="ops-panel registration-form-panel" data-reveal>
            ...
            <form className="onboarding-form" onSubmit={(event) => void handleWorkspaceSubmit(event)}>
              <label>
                <span>Workspace name</span>
                <input name="workspaceName" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} required />
              </label>
              <label>
                <span>Domain</span>
                <input name="workspaceDomain" value={workspaceDomain} onChange={(event) => setWorkspaceDomain(event.target.value)} required />
              </label>
              <label>
                <span>Owner email</span>
                <input name="adminEmail" type="email" value={adminEmail} onChange={(event) => setAdminEmail(event.target.value)} required />
              </label>
              <label>
                <span>Owner name</span>
                <input name="adminName" value={adminName} onChange={(event) => setAdminName(event.target.value)} required />
              </label>
              <label>
                <span>Plan</span>
              ...
              <button className="inline-action" disabled={!canRegisterWorkspace} type="submit">
                Create workspace
              </button>
            </form>
          ...
          <article className="ops-panel registration-form-panel" data-reveal>
            ...
            <form className="onboarding-form" onSubmit={(event) => void handleInviteSubmit(event)}>
              <label>
                <span>Workspace</span>
                <select name="inviteTenantId" value={inviteTenantId} onChange={(event) => setInviteTenantId(event.target.value)}>
              ...
              <label>
                <span>Email</span>
                <input name="inviteEmail" type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} required />
              </label>
              <label>
                <span>Role</span>
              ...
              <button className="inline-action" disabled={!canAdminMutate} type="submit">
                Create invite
              </button>
            </form>
          ...
          <article className="ops-panel registration-form-panel" data-reveal>
            ...
            <form className="onboarding-form" onSubmit={(event) => void handleClaimInvite(event)}>
              <label>
                <span>Invite preview</span>
                <select name="acceptInviteId" value={selectedInvite?.id ?? ''} onChange={(event) => setAcceptInviteId(event.target.value)} disabled={pendingInvites.length === 0}>
              ...
              <label>
                ...
                <input name="claimCode" value={claimCode} onChange={(event) => setClaimCode(event.target.value)} required />
              </label>
              <label>
                <span>Invite email</span>
                <input name="claimEmail" type="email" value={claimEmail} onChange={(event) => setClaimEmail(event.target.value)} required />
              </label>
              <label>
              ...
              <button className="inline-action" disabled={!canUseApi || !claimCode || !claimEmail} type="submit">
                Claim invite
              </button>
            </form>
            <button className="inline-action secondary-inline-action" disabled={!canAdminMutate || !selectedInvite} type="button" onClick={() => void acceptSelectedInvite()}>
              Admin accept selected
            </button>
            {selectedInvite && (
      ...
      <main className="product-main">
        ...
        <section className="admin-tabbar" role="tablist" aria-label="Admin sections">
          {adminTabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button key={tab.id} className="admin-tab" type="button" role="tab" aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)}>
                <Icon size={18} aria-hidden="true" />
                <span>{tab.label}</span>
              </button>
            );
...
function AccountActionCard({
  ...
  canMutate: boolean;
  onStatus: (status: AccountAction['status']) => Promise<void>;
  owner: string;
}) {
  return (
...
function NotificationEventCard({
  ...
  onStatus: (status: NotificationEvent['status']) => Promise<void>;
  owner: string;
}) {
  return (
...
function SuppressionRuleCard({
  ...
  canMutate: boolean;
  onStatus: (status: SuppressionRule['status']) => Promise<void>;
...
function GovernancePolicyCard({
  ...
  canMutate: boolean;
  onPolicy: (patch: Partial<GovernancePolicy>) => Promise<void>;
  policy: GovernancePolicy;
  updatedBy: string;
}) {
  return (
...
function RedactionRuleCard({
  ...
  canMutate: boolean;
  onStatus: (status: RedactionRule['status']) => Promise<void>;
  rule: RedactionRule;
}) {
  return (
    <div className="governance-card">
...
function DataRequestCard({
  ...
  canMutate: boolean;
  onStatus: (status: DataRequest['status']) => Promise<void>;
  request: DataRequest;
}) {
  return (
    <div className="governance-card">
